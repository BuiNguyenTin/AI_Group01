...
def astar(matrix, start_char, end_chars):
    if not matrix:
    ...
    for end_point in end_points:
        heapq.heappush(pq, (0, start_point, [], end_point))  # (f_cost, current_node, path, end_point)

    # Initialize visited set and cost dictionary
    visited = set()
    g_cost = {start_point: 0}
    f_cost = {start_point: heuristic_cost_estimate(start_point, end_points[0])}  # Initialize with first end_point

    # Perform A* search
    while pq:
        current_f_cost, (current_i, current_j), path, end_point = heapq.heappop(pq)
        ...
        for di, dj in directions:
            ...
            if 0 <= ni < nrows and 0 <= nj < ncols and (ni, nj) not in visited and matrix[ni][nj] != -1:
                ...
                if (ni, nj) not in g_cost or new_g_cost < g_cost[(ni, nj)]:
                    ...
                    heapq.heappush(pq, (f_cost[(ni, nj)], (ni, nj), path + [(current_i, current_j)], end_point))
...
def visualize_matrix_with_path(file_paths):
    ...
    for idx, file_path in enumerate(file_paths):
        ...
        # Find path using A* from 'P' to 'E' and from 'E' to 'G'
        if idx == 0:
            start_char = 'P'
            end_chars = ['E', 'G']
        ...
        # Find path using A*
        path = astar(matrix, start_char, end_chars)
        if path:
            for (i, j) in path:
