...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    '''
    Just like the single goal problem, we use the manhattan distance
    between the current state and the goal as the heuristic function.

    Let's assume that the manhattan distances between the current state
    and the four corners are four distinct heuristic functions: h1, h2, h3, h4.

    We know that it's best to choose a heuristc function with higher values.
    So in our case we choose the maximum of the heuristic values:

    >>> h = max(h1, h2, h3, h4)

    To optimize this further, we already know that states keep track of the visited corners,
